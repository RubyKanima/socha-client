...
class Logic(IClientHandler):
    ...
    def min_max(self, new_state: GameState, depth: int, alpha, beta, memo : dict = {}):
            ...
            if maximizing:
                ...
                for child in new_state.possible_moves:
                    eval = self.min_max(new_state.perform_move(child), depth - 1, alpha, beta, memo)
    ...
    def max_move(self):
        ...
        for child in self.game_state.possible_moves:
            if len(self.game_state.board.get_empty_fields()) > 68:
                mini_max = self.min_max(self.game_state.perform_move(child), 6, max_val, 100)
            if len(self.game_state.board.get_empty_fields()) > 55:
                mini_max = self.min_max(self.game_state.perform_move(child), 4, max_val, 100)
            elif len(self.game_state.board.get_empty_fields()) > 39:
                mini_max = self.min_max(self.game_state.perform_move(child), 3, max_val, 100)
            elif len(self.game_state.board.get_empty_fields()) > 33:
                mini_max = self.min_max(self.game_state.perform_move(child), 2, max_val, 100)
            else:
                mini_max = self.min_max(self.game_state.perform_move(child), 1, max_val, 100)
            val = mini_max
            if val > max_val:
                max_move = child
                max_val = val
